...
then
    echo 'usage:' 
    echo '	build-for-user.id <tag> <user> <filepath>'
    echo ''
    echo 'args:'
...
# . build-for-user.sh $tag $user

# # example: passing arguments explicity
# user=$(whoami);
# tag=image_for_$user:latest
# dockerfile_name=Dockerfile;
# . build-for-user.sh $tag $user $dockerfile
